    ...
    class BallRim {
        ...
        void move() {
            ...
            if ((xDir > 0) && (x + this.radius * 2 >= w)) {
                xDir = -1 * xDir;
                setCollisionText("え、痛いんだけど", 3);
            } else if ((xDir < 0) && (x <= 0)) {
                xDir = -1 * xDir;
                setCollisionText("ワイの左側変形しちゃう...", 6);
            } else if (xDir > 0) {
                x = x + 10;
            } else {
                x = x - 10;
            ...
            if ((yDir > 0) && (y + this.radius * 2 >= h)) {
                yDir = -1 * yDir;
                setCollisionText("顎折れるって", 3);
            }
            if ((yDir < 0) && (y <= 0)) {
                yDir = -1 * yDir;
                setCollisionText("やめてよ馬鹿になっちゃう...", 6);
            }
            ...
            if (strCounter > 0) {
                strCounter--;
                if (strCounter == 0) {
                    currentColor = basicColor; // 元の色に戻す
                }
            }
        }
